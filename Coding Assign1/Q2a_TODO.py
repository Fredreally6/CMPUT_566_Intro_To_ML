...
def train(X_train, y_train, X_val, y_val):
    ...
    for epoch in range(MaxIter):
        ...
        for b in range(int(np.ceil(N_train/batch_size))):
            ...
            w = w - alpha * (1/len(y_batch)) * X_batch.T @ (y_hat_batch - y_batch)  # the gradient of loss is 1/M*XT(Xw - t)
...
# TODO: Your code here
best_epoch, best_risk, best_weights, losses_train, risks_val = train(X_train, y_train, X_val, y_val)
...
plt.savefig('validation risk&epoch.jpg')

plt.figure()
plt.plot(range(MaxIter), losses_train)
plt.xlabel('Number of epochs')
plt.ylabel('Training loss')
plt.savefig('training loss&epoch.jpg')